...
import multiprocessing, Queue, sys, argparse, os, subprocess, re
from lib import oak
...
class basioConverter(multiprocessing.Process):
    ...
    def run(self):
        ...
        while not self.kill_received:
            ...
            previouscp = None
            contig = StringIO(unicode(contig) )
            for changepoint in sgmblock["changepoints"]:
                if previouscp:
                    segment = contig.read( changepoint - previouscp )
                    oak.add_segment(self.shrub, segment)
                previouscp = changepoint
            # and fill the blanks if any
...
if __name__ == '__main__':
    ...
    argparser.add_argument("-f", "--fastafile", help="Multifasta file path (default: deduced from sgm file name)", default=None)
    argparser.add_argument("sgmfile", help=".sgm file path", default="./")
    args = argparser.parse_args()
    
    fastafile = args.fastafile
    if not args.fastafile:
        fastafile = re.sub(r'_[a-zA-Z]+[0-9\.]+\.[a-z]+$', '.fasta', args.sgmfile)
        pass 
    ...
    print "Loading queue..."
    sgmfile = open(args.sgmfile, "r")
    sgmblock = None
    for line in sgmfile.readlines():
        if line.startswith(">"):
            if sgmblock:
            ...
            line = line.split()
            if "changepoints" in sgmblock.keys():
                sgmblock["changepoints"].append(int(line[1]))
            else:
                sgmblock["changepoints"] = [int(line[1])]
    work_queue.put(sgmblock)
    queue_length = queue_length +1
...
#        oak.save_to_file(distree, os.path.splitext(args.sgmfile)[0], args.isreference)
        oak.save_to_file(distree, os.path.splitext(args.sgmfile)[0])
#        file1 = open("%s.mvseg" % os.path.splitext(args.sgmfile)[0], "w")
#        seglist = distree[2].keys()
#        for i in range(len(seglist)):
#            file1.write("%d %s\n" % (i, seglist[i]))
#        file1.close()
#        file2 = open("%s.mvdist" % os.path.splitext(args.sgmfile)[0], "w")
