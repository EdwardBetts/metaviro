...
class _base_LMNN(BaseMetricLearner, BaseEstimator, TransformerMixin):
  def __init__(self, **kwargs):
    #self.params = kwargs
    # LA for sklearn
    self.k = kwargs.get('k',"default value")
    self.min_iter = kwargs.get('min_iter')
    self.max_iter = kwargs.get('max_iter')
    self.learn_rate = kwargs.get('learn_rate')
    self.regularization = kwargs.get('regularization')
    self.convergence_tol = kwargs.get('convergence_tol')
...
try:
  ...
  print "SHOGUN found, using the fast C++ version"
  ...
  LMNN = python_LMNN
  print "SHOGUN not found, using the slow Python version"
